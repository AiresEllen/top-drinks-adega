...
export default function PedidosPage() {
  ...
  async function cleanupOldArchivedOrders() {
    ...
    if (error) {
      console.error("Erro ao verificar arquivados antigos:", error);
      return;
    }

    const oldArchivedIds = (data || [])
      .filter((order: any) => {
        const status = normalizeStatus(order.status);
        if (status !== "arquivado") return false;

        const baseDate = order.updated_at || order.created_at;
        if (!baseDate) return false;

        return new Date(baseDate) < limitDate;
      })
      .map((order: any) => order.id);

    if (oldArchivedIds.length === 0) return;

    const { error: deleteError } = await supabase
      .from("orders")
      .delete()
      .in("id", oldArchivedIds);

    if (deleteError) {
      console.error(
        "Erro ao excluir arquivados com mais de 30 dias:",
        deleteError,
      );
    }
  }
  ...
  async function fetchOrders(showAlertForNew = false) {
    await cleanupOldArchivedOrders();
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">
                Onde encontrar os arquivados
              </h2>
              <p className="text-sm text-slate-500">
                Você pode abrir pela aba <strong>Arquivados</strong> ou por esta
                rota: <strong>/admin/pedidos/arquivados</strong>.
              </p>
            </div>

            <Link
              href="/admin/pedidos/arquivados"
              className="rounded-2xl bg-slate-800 px-5 py-3 text-center font-semibold text-white transition hover:bg-slate-900"
            >
              Abrir arquivados
            </Link>
          </div>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            filteredOrders.map((order) => {
              ...
              const canArchive =
                normalized === "entregue" || normalized === "cancelado";
              const canDelete =
                normalized === "entregue" ||
                    ...
                    <div className="flex w-full flex-col gap-3 xl:w-[340px]">
                      ...
                      {canArchive && (
                        <button
                          type="button"
                          onClick={() =>
                            updateOrderStatus(order.id, "arquivado")
                          }
                          disabled={savingId === order.id}
